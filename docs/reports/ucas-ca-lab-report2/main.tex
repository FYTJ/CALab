...
\lstdefinelanguage{Chisel}{
    ...
    morekeywords=[2]{Boolean, Byte, Char, Double, Float, Int, Long, Short, String, Unit, 
                    Any, AnyRef, AnyVal, Nothing, Null, Option, Some, None, List, Array, 
                    Map, Set, Seq, Vector, UInt, SInt, Bool, Bundle, Vec, Module, IO, 
                    Input, Output, Flipped, Decoupled, Wire, Reg, RegInit, RegNext, RegEnable, 
                    Clock, Reset, AsyncReset, Data, Bits, Aggregate},
    morekeywords=[3]{printf, assert, assume, cover, when, elsewhen, otherwise, switch, 
                    is, Cat, Fill, Mux, PriorityMux, PriorityEncoder, OHToUInt, UIntToOH, 
                    PopCount, Reverse, rotateLeft, rotateRight, asUInt, asSInt, asBool, 
                    asTypeOf, chiselTypeOf, WireInit, RegInit, DontCare},
    morekeywords=[4]{:=, <>, =/=, ===, =/==, andR, orR, xorR, +&, -&, *&},
    morecomment=[l]{//},
    morecomment=[s]{/*}{*/},
    morestring=[b]",
    morestring=[b]',
...
% \section{新的发现}
% \subsection{问题的提出}
% 在完成本次实验设计后，组内同学总结复盘时，感觉当前CPU与乘除法器模块的握手信号设计仍有优化空间，逻辑能够更加严密自然，遂试图寻找新的方案，却不慎制造出了组合逻辑环。

% 最终我们解决了问题，提出了如下能在日后具体实施的方案：

% \subsection{方案背景}
% 对于模块而言，每个模块只能看到自身内部的状态，而不可见其他模块的状态。即：假设有两个独立模块A和B，B的状态A完全不可见，同理A的状态B完全不可见。两者之间的交流需要完全依靠握手信号。

% 以CPU和乘法器为例：CPU采用5级流水：IF, ID, EX, MEM, WB；乘法器采用2级流水：M1, M2。在EX阶段，CPU向M1发送乘法操作码和操作数；在MEM阶段，M2向CPU返回计算结果。CPU不知道乘法器的运算情况，乘法器也不知道CPU的每个阶段都在做什么，因此要有及其合理的握手信号进行交互。

% \subsection{不慎造出组合逻辑环的情况}
% 按照我们的流水控制，EX_ready_go表示CPU的EX级任务是否完成，若EX级握手不成功，则EX级未完成任务，需要被阻塞，因此有
% \begin{lstlisting}[language=Verilog]
% // 在EX级内
% wire ready_go = to_mul_req_valid && from_mul_req_ready; // 握手完成
% wire to_mul_req_valid = in_valid;  // 在M1视角下，该信号为from_cpu_req_valid; in_valid用于表示当前阶段是否有效
% \end{lstlisting}
% 由于模块间设计的对称性，在M1级内，ready_go信号也应赋值为握手完成(fire)
% \begin{lstlisting}[language=Verilog]
% // 在M1级内
% wire ready_go = to_mul_resp_valid && from_mul_resp_ready; // 握手完成
% wire from_cpu_req_ready = in_ready;  // 在CPU视角下，该信号为to_mul_req_ready
% \end{lstlisting}
% 进而遇到了组合逻辑环(记信号a依赖于信号b为'a <- b')：

% M1_ready_go <- from_cpu_req_ready <- M1_in_ready <- M1_ready_go

% \subsection{新的思路：将from_cpu_req_ready与in_ready解耦}
% 即：断开上文中from_cpu_req_ready 与 M1_in_ready之间的箭头

% 为什么这种思路是合理的呢？我们不妨考虑一种比“从模块”流水线非对齐更复杂的情况。

% 对于更复杂的设计，我们应该抛弃“流水线”、“子模块”的概念，将“流水线”仅视作一种优化技术，而将整个设计视作由多个子系统(subsystem)组成。这样的理解，要求我们必须维持子系统中信号的对称性。考虑CPU和内存为两个子系统，CPU和内存哪个作为“从模块”都不合适，二者应当视作彼此独立的子系统。

% 作为示例，我们考虑两条独立的流水线，假设EX1向EX2发送信息，MEM1从MEM2接收信息。现在考虑EX2中的in_ready信号，该信号表示EX2是否准备好接收ID2的数据，而握手是EX2级的内部任务，因此我们可以立即发现，to_1_req_ready = in_ready的赋值是不合理的。我们再考虑从EX2向MEM2的数据传输，如果该数据通路被阻塞，则握手成功后EX2接收到的数据将无处可存放。MEM2无法接受数据，意味着EX2的out_ready=0，因此正确的逻辑是to_1_req_ready = out_ready。因为out_ready是后续流水级产生的数据，因此一定不会出现组合逻辑环。
